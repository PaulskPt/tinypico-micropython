...
# See: https://github.com/boochow/MicroPython-ST7735/blob/master/ST7735.py
# Modifications by @PaulskPt
# - default screen dimensions changed to 160 x 80
# - moved the ScreenSize definition inside class TFT
# - added definitions hori = 0 and vert = 1 to make script more readable
# - added optional parameters  width=ScreenSize[hori], height=ScreenSize[vert]) to __init__()
# - added function get_size()
# - added function get_rotation()
# - added to __init__() definition  self.vscroll_max
import machine
import time
...
class TFT(object) :
  ...
  ScreenSize = (160, 80) # was (128, 160)  (width, height)
  ...
  # params width and height added by @Paulskpt to have possibility to set them from the calling program
  def __init__( self, spi, aDC, aReset, aCS, width=ScreenSize[hori], height=ScreenSize[vert]) :
    ...
    self.vscroll_max = self._size[vert] + 2 # added by @Paulskpt. See setvscroll() and vscroll()
  ...
  def set_size( self, width, height ):
      self._size = (width, height)
      print("ST7735::set_size(): new dimensions: ", width, height, self._size)
      
  def get_offset( self ):  # function added by @PaulskPt
      lst = [b for b in self._offset]
      return tuple(lst)  # self._offset
    
  def set_offset( self, x, y):
      TAG = "set_offset(): "
      t = "cannot set new offset. Reason:\n"
      if isinstance(x, int) and isinstance(y, int):
          if x >= 0 and x < self._size[hori]:
              if y >= 0 and y < self._size[vert]:
                  self._offset = bytearray([x,y])
                  print(TAG+"new offset(hor, vert) = ", self._offset)
              else:
                  print(TAG+t+"\ty dimension {}, is out of screen y dimension {}".format(y, self._size[vert]))
          else:
              print(TAG+t+"\tx dimension {} is out of screen x dimension {}".format(x, self._size[hori]))
      else:
          print(TAG+t+"\tParameters x and y must be integers. Parameters given: ", type(x), type(y))
...
#   @micropython.native
  def rotation( self, aRot ) :
    ...
    if (0 <= aRot < 4):
      ...
      """
      if (rotchange & 1):
        self._size =(self._size[vert], self._size[hori])
      """
      self._setMADCTL()
      
  def get_rotation(self):  # added by @Paulskpt
      return self.rotate

  @micropython.native
  def pixel( self, aPos, aColor ) :
    '''Draw a pixel at the given position'''
    if 0 <= aPos[hori] < self._size[hori] and 0 <= aPos[vert] < self._size[vert]:
...
#   @micropython.native
  def text( self, aPos, aString, aColor, aFont, aSize = 1, nowrap = False ) :
    '''Draw a text at the given position.  If the string reaches the end of the
       display it is wrapped to aPos[hori] on the next line.  aSize may be an integer
       which will size the font uniformly on w,h or a or any type that may be
       indexed with [hori] or [vert].'''
    ...
    px, py = aPos
    width = wh[hori] * aFont["Width"] + 1
    for c in aString:
      self.char((px, py), c, aColor, aFont, wh)
      px += width
      #We check > rather than >= to let the right (blank) edge of the
      # character print off the right of the screen.
      if px + width > self._size[hori]:
        if nowrap:
          break
        else:
          py += aFont["Height"] * wh[vert] + 1
          px = aPos[hori]
...
#   @micropython.native
  def char( self, aPos, aChar, aColor, aFont, aSizes ) :
    ...
    if (startchar <= ci <= endchar):
      ...
      px = aPos[hori]
      if aSizes[hori] <= 1 and aSizes[vert] <= 1 :
        ...
        for q in range(fontw) :
          ...
          for r in range(fonth) :
            ...
            c >>= 1
        self.image(aPos[hori], aPos[vert], aPos[hori] + fontw - 1, aPos[vert] + fonth - 1, buf)
      else:
        for c in charA :
          py = aPos[vert]
          for r in range(fonth) :
...
#   @micropython.native
  def line( self, aStart, aEnd, aColor ) :
    ...
    if aStart[hori] == aEnd[hori]:
      #Make sure we use the smallest y.
      pnt = aEnd if (aEnd[vert] < aStart[vert]) else aStart
      self.vline(pnt, abs(aEnd[vert] - aStart[vert]) + 1, aColor)
    elif aStart[vert] == aEnd[vert]:
      #Make sure we use the smallest x.
      pnt = aEnd if aEnd[hori] < aStart[hori] else aStart
      self.hline(pnt, abs(aEnd[hori] - aStart[hori]) + 1, aColor)
    else:
      px, py = aStart
      ex, ey = aEnd
      dx = ex - px
...
#   @micropython.native
  def vline( self, aStart, aLen, aColor ) :
    ...
    start = (clamp(aStart[hori], 0, self._size[hori]), clamp(aStart[vert], 0, self._size[vert]))
    stop = (start[hori], clamp(start[vert] + aLen, 0, self._size[vert]))
    #Make sure smallest y 1st.
    if (stop[vert] < start[vert]):
...
#   @micropython.native
  def hline( self, aStart, aLen, aColor ) :
    ...
    start = (clamp(aStart[hori], 0, self._size[hori]), clamp(aStart[vert], 0, self._size[vert]))
    stop = (clamp(start[hori] + aLen, 0, self._size[hori]), start[vert])
    #Make sure smallest x 1st.
    if (stop[hori] < start[hori]):
      start, stop = stop, start
    self._setwindowloc(start, stop)
    self._setColor(aColor)
    self._draw(aLen)
...
#   @micropython.native
  def rect( self, aStart, aSize, aColor ) :
    ...
    self.hline(aStart, aSize[hori], aColor)
    self.hline((aStart[hori], aStart[vert] + aSize[vert] - 1), aSize[hori], aColor)
    self.vline(aStart, aSize[vert], aColor)
    self.vline((aStart[hori] + aSize[hori] - 1, aStart[vert]), aSize[vert], aColor)
...
#   @micropython.native
  def fillrect( self, aStart, aSize, aColor ) :
    '''Draw a filled rectangle.  aStart is the smallest coordinate corner
       and aSize is a tuple indicating width, height.'''
    start = (clamp(aStart[hori], 0, self._size[hori]), clamp(aStart[vert], 0, self._size[vert]))
    end = (clamp(start[hori] + aSize[hori] - 1, 0, self._size[hori]), clamp(start[vert] + aSize[vert] - 1, 0, self._size[vert]))

    if (end[hori] < start[hori]):
      tmp = end[hori]
      end = (start[hori], end[vert])
      start = (tmp, start[vert])
    if (end[vert] < start[vert]):
      tmp = end[vert]
      end = (end[hori], start[vert])
      start = (start[hori], tmp)

    self._setwindowloc(start, end)
    numPixels = (end[hori] - start[hori] + 1) * (end[vert] - start[vert] + 1)
    self._setColor(aColor)
...
#   @micropython.native
  def fillcircle( self, aPos, aRadius, aColor ) :
    ...
    for x in range(aRadius) :
      ...
      y0 = aPos[vert] - y
      ey = y0 + y * 2
      y0 = clamp(y0, 0, self._size[vert])
      ln = abs(ey - y0) + 1;

      self.vline((aPos[hori] + x, y0), ln, aColor)
      self.vline((aPos[hori] - x, y0), ln, aColor)

  def fill( self, aColor = WHITE ) :
  ...
  def setvscroll(self, tfa, bfa) :
    ...
    data2[1] = self.vscroll_max - tfa - bfa
    self._writedata(data2)
    data2[1] = bfa
    self._writedata(data2)
    self.tfa = tfa
  ...
  def vscroll(self, value) :
    a = value + self.tfa
    if (a + self.bfa > self.vscroll_max) :
      a = 162 - self.bfa
    self._vscrolladdr(a)
...
#   @micropython.native
  def _setwindowpoint( self, aPos ) :
    ...
    x = self._offset[hori] + int(aPos[hori])
    y = self._offset[vert] + int(aPos[vert])
    self._writecommand(TFT.CASET)            #Column address set.
    self.windowLocData[0] = self._offset[hori]
    self.windowLocData[1] = x
    self.windowLocData[2] = self._offset[hori]
    self.windowLocData[3] = x
    self._writedata(self.windowLocData)

    self._writecommand(TFT.RASET)            #Row address set.
    self.windowLocData[0] = self._offset[vert]
    self.windowLocData[1] = y
    self.windowLocData[2] = self._offset[vert]
...
#   @micropython.native
  def _setwindowloc( self, aPos0, aPos1 ) :
    ...
    self._writecommand(TFT.CASET)            #Column address set.
    self.windowLocData[0] = self._offset[hori]
    self.windowLocData[1] = self._offset[hori] + int(aPos0[hori])
    self.windowLocData[2] = self._offset[hori]
    self.windowLocData[3] = self._offset[hori] + int(aPos1[hori])
    self._writedata(self.windowLocData)

    self._writecommand(TFT.RASET)            #Row address set.
    self.windowLocData[0] = self._offset[vert]
    self.windowLocData[1] = self._offset[vert] + int(aPos0[vert])
    self.windowLocData[2] = self._offset[vert]
    self.windowLocData[3] = self._offset[vert] + int(aPos1[vert])
    self._writedata(self.windowLocData)
  ...
  def _setMADCTL( self ) :
    ...
    rgb = TFT.TFTRGB if self._rgb else TFT.TFTBGR
    self._writedata(bytearray([TFT.TFTRotations[self.rotate] | rgb]))
  ...
  def initb( self ) :
    ...
    self._size = (ScreenSize[hori] + 2, ScreenSize[vert] + 1)
    self._reset()
    ...
    self.windowLocData[3] = self._size[hori] - 1
    self._writedata(self.windowLocData)

    self._writecommand(TFT.RASET)                #Row address set.
    self.windowLocData[1] = 1                    #Start at row 2.
    self.windowLocData[3] = self._size[vert] - 1
    self._writedata(self.windowLocData)
